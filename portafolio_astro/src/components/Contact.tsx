    ...
    <section id="contact" className="section-container">
      <div className="section-content">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="text-center mb-16">
          <h2 className="modern-heading text-4xl lg:text-5xl">{isEN ? 'Contact' : 'Contacto'}</h2>
          <p className="text-xl max-w-3xl mx-auto" style={{ color: 'var(--neo-muted)' }}>
            {isEN ? 'Got a project in mind? Let’s build it together!' : '¿Tienes un proyecto en mente? ¡Construyámoslo juntos!'}
          </p>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 max-w-6xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="neo-card">
            <h3 className="text-2xl font-bold mb-8" style={{ color: 'var(--neo-text)' }}>{isEN ? 'Send me a message' : 'Envíame un mensaje'}</h3>
            <form className="space-y-6">
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--neo-muted)' }}>{isEN ? 'Name' : 'Nombre'}</label>
              ...
              <div>
                <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--neo-muted)' }}>{isEN ? 'Subject' : 'Asunto'}</label>
                <input type="text" className="neo-input" placeholder={isEN ? 'How can I help?' : '¿En qué puedo ayudarte?'} />
              </div>
              <div>
                <label className="block text-sm font-semibold mb-2" style={{ color: 'var(--neo-muted)' }}>{isEN ? 'Message' : 'Mensaje'}</label>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="space-y-6">
            <div className="neo-card">
              <h3 className="text-2xl font-bold mb-4" style={{ color: 'var(--neo-text)' }}>{isEN ? 'Information' : 'Información'}</h3>
            ...
            <div className="neo-card">
              <h3 className="text-2xl font-bold mb-4" style={{ color: 'var(--neo-text)' }}>{isEN ? 'Resume' : 'Currículum'}</h3>
              <a href="/AlexisPalacio_ATS.pdf" target="_blank" rel="noopener noreferrer" className="neo-btn inline-flex items-center gap-2">{isEN ? 'Download CV (PDF)' : 'Descargar CV (PDF)'}</a>
            </div>
            <div className="neo-card">
              <h3 className="text-2xl font-bold mb-4" style={{ color: 'var(--neo-text)' }}>{isEN ? 'Networks' : 'Redes'}</h3>
